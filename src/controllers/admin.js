...
class Admin extends BaseClass {
  ...
  async update() {
    const { email, name, profile_photo, address, phone } =
    ...
    const savedAdmin = await this.models.Admin.findOne({
      _id: admin._id,
    }).populate("address");
  ...
  async updateBookingStatus() {
    ...
    const updatedBooking = await this.models.Bookings.findOne({
      _id: booking._id,
    })
      .populate("user")
      .populate("hotel")
      .populate("room")
      .select(
        "user hotel _id booking_status room check_in check_out totalPrice"
      );
